...
class server_biggi{
	function svr_print_adicional($empresa, $nro_orden_compra){
		...
		if($empresa == 'BODEGA'){
			$sql = "SELECT U.INI_USUARIO+'|'+CONVERT(VARCHAR, OC.COD_ORDEN_COMPRA)+'|'+CONVERT(VARCHAR, SC.COD_SOLICITUD_COMPRA) RESULTADO
	...
	function svr_print_etiqueta($nro_orden_compra){
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		$sql = "SELECT U.INI_USUARIO+'|'+CONVERT(VARCHAR, OC.COD_ORDEN_COMPRA)+'|'+'NO ASOCIADO'+'|'+CONVERT(VARCHAR, SC.COD_SOLICITUD_COMPRA) RESULTADO
				FROM ORDEN_COMPRA OC
					,SOLICITUD_COMPRA SC
					,USUARIO U
				WHERE OC.COD_ORDEN_COMPRA = $nro_orden_compra
