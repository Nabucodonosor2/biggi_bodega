...
class wo_inf_bodega_por_recibir extends w_informe_pantalla {
   ...
   function wo_inf_bodega_por_recibir() {
		...
		$this->add_header(new header_num('PRECIO_COMPRA_RECIBIR', 'PRECIO_COMPRA_RECIBIR', 'Costo x recibir', 0, true, 'SUM'));
		...
		$this->dw->add_control(new static_num('PRECIO_COMPRA_RECIBIR'));
   	}
	...
	function print_inf($solicitud_compra = '') {
		...
		$this->nom_filtro .= "Fecha impresión = ".$this->current_date()."; ";		
		$this->nom_filtro .= "Cod. Solicitud Compra = $param_solicitud";		
