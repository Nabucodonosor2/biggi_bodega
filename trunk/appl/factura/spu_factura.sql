                        ...
                        begin
                            if(@vl_porc_iva = 0)--FACTURA EXENTA mu    
                                ...
                                begin
                                    ...
                                    if(@vl_count > 0)
                                        begin
                                            select @vl_nro_factura = max(nro_factura)
                                            from factura
											where nro_factura < 77300
											and cod_estado_doc_sii = @kl_cod_estado_fa_enviada
                                    		and porc_iva <> 0
											
											if(@vl_nro_factura = 77299)
                                            	select @vl_nro_factura = max(nro_factura) + 1
	                                            from factura
												where cod_estado_doc_sii = @kl_cod_estado_fa_enviada
	                                    		and porc_iva <> 0
