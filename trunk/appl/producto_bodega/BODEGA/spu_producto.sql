...
BEGIN
		if (@ve_operacion='UPDATE')		
			begin
				declare
					@vl_precio_venta_pub_old	numeric,
					@vl_precio_venta_int_old	numeric,
					@vl_count					numeric
								
				/*Cuando se cambia un Precio venta (Publico o Interno) en Bodega Biggi, se debe 
				 actualizar el mismo campo precio venta(Publico o Interno) en BD “BIGGI”
				 ,tabla producto.*/
				...
				if(@vl_count > 0)BEGIN
					...
					select @vl_precio_venta_pub_old = PRECIO_VENTA_PUBLICO
						  ,@vl_precio_venta_int_old = PRECIO_VENTA_INTERNO
					from PRODUCTO_BIGGI
					where cod_producto = @ve_cod_producto
					
					if(@ve_precio_venta_interno <> @vl_precio_venta_int_old)
						UPDATE PRODUCTO_BIGGI
						SET PRECIO_VENTA_INTERNO = @ve_precio_venta_interno
						WHERE COD_PRODUCTO = @ve_cod_producto
						
					if(@ve_precio_venta_publico <> @vl_precio_venta_pub_old)
						UPDATE PRODUCTO_BIGGI
						SET PRECIO_VENTA_PUBLICO = @ve_precio_venta_publico
						WHERE COD_PRODUCTO = @ve_cod_producto	
