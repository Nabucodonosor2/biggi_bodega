...
class wo_guia_despacho extends w_output {
	...
	function wo_guia_despacho() {
		...
		parent::w_output('guia_despacho', $sql, $_REQUEST['cod_item_menu']);
		...
		$this->add_header(new header_num('NRO_FACTURA', 'F.COD_FACTURA', 'Nº Factura'));
		...
		$this->add_header(new header_drop_down_string('TIPO_GD', '(select dbo.f_tipo_fa(EDS.NOM_ESTADO_DOC_SII))', 'Tipo GD', $sql));
  	}
