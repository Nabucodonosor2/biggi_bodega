...
class wi_entrada_bodega extends wi_entrada_bodega_base{
	...
	function new_record() {
		...
		$this->dws['dw_entrada_bodega']->set_entrable('TIPO_DOC', false);
	}
		...
		if ($db->EXECUTE_SP($sp, $param)){
			...
			if ($tipo_doc != 'AJUSTE'){
				...
				if($result_emp[0]['COD_EMPRESA'] <> 4 && $result_emp[0]['COD_EMPRESA'] <> 5){//4: Todoinox	5:Servindus
					...
					if($result_emp[0]['COD_EMPRESA'] == 5){  //DEJAR SOLO PARA SERVINDUS TODOINOX LO HACE DESDE WS
						//se crea Factura proveedor 
						$tipo_fa_proveedor = session::get("TIPO_FA_PROVEEDOR");
						session::un_set("TIPO_FA_PROVEEDOR");
						$tipo_fa_proveedor = ($tipo_fa_proveedor=='') ? 'NULL' : $tipo_fa_proveedor;
...
class print_entrada_bodega extends reporte_biggi {	
	...
	function modifica_pdf(&$pdf){
	    ...
	    $fecha_impresion = $result[0]['FECHA_IMPRESION'];
	    $hora_impresion = $result[0]['HORA_IMPRESION'];
	    
	    $pdf->SetFont('','',10);
	    $pdf->SetTextColor(0, 0, 0);
	    $pdf->SetXY(454, 93);
	    $pdf->Cell(70,15,'Fecha Impresión', 0, '','L');
	    $pdf->SetXY(535, 93);
	    $pdf->Cell(70,15,$fecha_impresion, 0, '','L');
	    
	    $pdf->SetXY(454, 109);
	    $pdf->Cell(70,15,'Hora Impresión', 0, '','L');
	    $pdf->SetXY(535, 109);
	    $pdf->Cell(70,15,$hora_impresion, 0, '','L');
	}
}
?>